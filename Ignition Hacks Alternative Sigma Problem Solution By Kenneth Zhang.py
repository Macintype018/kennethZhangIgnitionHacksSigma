...
tokenizer.fit_on_texts(trainingData['Text'].values)
X = tokenizer.texts_to_sequences(trainingData['Text'].values)
X = pad_sequences(X)
...
model = Sequential()
model.add(Embedding(max_features, embdim,input_length = X.shape[1]))
...
contestantText = tokenizer.texts_to_sequences(twt)
